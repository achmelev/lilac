...
class LilacLexer(RegexLexer):
    ...
    tokens = {
        'root': [
            ...
            (r'//.*?\n', Comment.Single),
            (r'/\*.*?\*/', Comment.Multiline),
			(r'(abstract|all|annotation|append|argument|array|at|attribute|bootstrap|bound|bridge|cast|catch|chop|class|code|const|constant|constructor|debug|default|deprecated|descriptor|element|enclosing|enum|extended|extends|field|file|final|formal|full|go|implements|index|inner|interface|invisible|line|locals|maxlocals|maxstack|method|name|native|nested|newinvokespecial|normal|numbers|outer|parameter|path|private|protected|public|receiver|reference|resource|same|signature|source|stackmap|static|strict|super|supertype|synchronized|synthetic|target|targeted|targets|throws|to|transient|try|type|types|unknown|value|var|varargs|vars|version|volatile)\b',Keyword),
            (r'(boolean|byte|char|classref|double|dynref|fieldref|float|int|intfmethodref|long|methodhandle|methodref|methodtype|nameandtype|null|object|returnadress|short|string|top|uninitialized|uninitializedthis|utf8)\b',Keyword.Type),
